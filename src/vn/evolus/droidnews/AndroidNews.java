...
import vn.evolus.droidnews.adapter.ChannelAdapter;
...
import vn.evolus.droidnews.services.ImagesDownloadingService;
	...
	private void refreshUnreadCounts() {		
    	...
    	refreshTask.setCallable(new BetterAsyncTaskCallable<Void, Void, Void>() {
			public Void call(BetterAsyncTask<Void, Void, Void> arg0)
					throws Exception {
				Map<Long, Integer> unreadCounts = ContentManager.countUnreadItemsForEachChannel(cr);
				for (Channel channel : channels) {
					try {				
